        ...
        for i = 1:length(conformations)
            ...
            if isequal(func,@normalize)
                if ~test
                    ...
                    skelldef.template = makethinskel(normalize(makefatskel(template),skelldef));
                else
                    %%%
                    ...
                    %disp('')
                    alpha = sum(conformstruc.data.'*extskeldef.template)/size(conformstruc.data,2)/(extskeldef.template.'*extskeldef.template);
                    skelldef.pos_std = 1;% alpha*8; %%% my normalizing function didnt work, this was fitted by hand
                    skelldef.pos_mean= 0 ;
                    skelldef.vel_std = 1;% alpha*8; %%% no idea if this will have the same scaling factor!
                    skelldef.vel_mean=0;
                end
            end
